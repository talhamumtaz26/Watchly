...
const Login = () => {
  ...
  const { login, signup, signInWithGoogle, authLoading } = useAuth();
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // For redirect flow (mobile), the page will reload automatically
      // For popup flow (desktop), onAuthStateChanged will trigger
    } catch (error) {
      console.error('Google sign in error:', error);
      // Only show error if it's not a popup close or redirect
  ...
  return (
    <div className="fixed inset-0 bg-background-darker flex items-center justify-center px-4 overflow-hidden">
      ...
      {/* Show loading overlay when processing redirect */}
      {authLoading && (
        <div className="absolute inset-0 bg-background-darker/90 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="text-center">
            <svg className="animate-spin h-12 w-12 text-accent-red mx-auto mb-4" viewBox="0 0 24 24">
